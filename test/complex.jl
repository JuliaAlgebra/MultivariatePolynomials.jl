@testset "ComplexPoly" begin
    ...
    @test !isrealpart(x) && !isrealpart(conj(x)) && isrealpart(real(x)) && !isrealpart(imag(x))
    @test !isimagpart(x) && !isimagpart(conj(x)) && !isimagpart(real(x)) && isimagpart(imag(x))
    @test !isconj(x) && isconj(conj(x)) && !isconj(real(x)) && !isconj(imag(x))
    @test ordinary_variable(conj(x)) == ordinary_variable(real(x)) == ordinary_variable(imag(x)) == ordinary_variable(x)
    ...
    @test conj(7x^3 + 6y^2 - (3+im)x + 8a) == 7conj(x)^3 + 6conj(y)^2 - (3-im)*conj(x) + 8a
    @test conj(monomial_vector([x, y, x*y^3, y*x^4])) == monomial_vector([conj(x), conj(y), conj(x*y^3), conj(y*x^4)])
    @test real(x*y^2*a) == (real(x) * real(y)^2 - imag(x) * 2*real(y)*imag(y) - real(x) * imag(y)^2) * a
    @test imag(x*y^2*a) == (imag(x) * real(y)^2 + real(x) * 2*real(y)*imag(y) - imag(x) * imag(y)^2) * a
    ...
    @test subs(4x + 8y^2 - 6x^3, [x, y] => [2 + 4im, 9 - im]) == (1176-32im)*x^0
    @test subs(4x + 8y^2 - 6x^3, [x, conj(y)] => [2 + 4im, 9 - im]) == (1176+256im)*x^0
    @test subs(4x + 8y^2 - 6x^3, [x, real(y)] => [2 + 4im, 9]) broken=true
    @test subs(4x + 8y^2 - 6x^3, [x, real(y)] => [2 + 4im, 9 + 0*x^0]) == 1184+112im + 144im*imag(y) - 8imag(y)^2
end
